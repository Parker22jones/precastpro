...
class _PhaseTakeoffState extends State<PhaseTakeoff> {
  ...
  Future<void> _exportSubmittal() async {
    ...
    try {
      final elevationPng =
          await renderPainterToPng(buildElevationPainter(design), const Size(640, 900));
      final planPng = await renderPainterToPng(buildPlanPainter(design), const Size(640, 640));
      ...
      final safeName =
          '${design.jobName} ${design.structureMark}'.replaceAll(RegExp(r'[^A-Za-z0-9_\- ]'), '').trim();
          ...
          children: [
            ...
            GridRow(
              ...
              cells: [
                ...
                (
                  const Text('TOTAL STRUCTURAL WEIGHT',
                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
                  6
                ),
                (Text('${design.stack.totalPieceCount}', style: Mh.cellNum), 1),
                (
                  Text(design.stack.achievedHeightIn.toStringAsFixed(0), style: Mh.cellNum),
                  2
                ),
                (const Text('', style: Mh.cell), 2),
                (
                  Text(_lbs(design.totalWeightLbs),
  ...
  Widget build(BuildContext context) {
    ...
    return GridRow(
      ...
      cells: [
        ...
        (
          StatusChip(
              label: component.status.label, color: statusColor(component.status), dense: false),
          3
        ),
        (
          component.status.next == null
              ...
              : TextButton(
                  ...
                  child: Text(component.status.next!.label.toUpperCase()),
                ),
          2
        ),
      ],
