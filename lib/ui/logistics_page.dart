        ...
        if (wide) {
          return Row(
            ...
            children: [
              ...
              Expanded(
                child: _StructureDetail(app: app, record: record, scrollable: true),
              ),
            ],
          );
        }
        // Narrow: one page-level scroll view holding both blocks, so every
        // component row stays reachable on a phone.
        return ListView(
          children: [
            _StructureList(
              structures: structures,
            ...
            _StructureDetail(app: app, record: record, scrollable: false),
          ],
        );
      },
    );
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      StatusChip(
                        label: record.rollupStatus.label,
                        color: statusColor(record.rollupStatus),
                      ),
                    ],
                  ),
                ),
...
class _StructureDetail extends StatelessWidget {
  const _StructureDetail({required this.app, required this.record, required this.scrollable});
  ...
  Widget build(BuildContext context) {
    final allShipped =
        record.components.isNotEmpty && record.components.every((c) => c.status.hasLeftYard);
...
/// Expands inside a bounded column, or lays out at its natural height when the
/// surrounding page already scrolls.
class _Flex extends StatelessWidget {
  const _Flex({required this.scrollable, required this.child, this.scrollChild = false});
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(child: scrollChild ? SingleChildScrollView(child: child) : child);
  }
}
